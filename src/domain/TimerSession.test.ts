...
describe('TimerSession', () => {
	...
	it('includes earlier offset in elapsed time', () => {
		const session = TimerSession.create(100, 30 * 60, new Date('2026-06-15T12:00:00.000Z'));
	...
	it('preserves an existing endedAt value when ending a session', () => {
		const session = TimerSession.from({
			...TimerSession.create(100, 0, new Date('2026-06-15T12:00:00.000Z')),
	...
	it('detects sessions started at least 24 real hours ago', () => {
		const session = TimerSession.create(100, 90 * 60, new Date('2026-06-14T12:00:00.000Z'));
	...
	it('uses stable UUIDs from the platform', () => {
		...
		expect(TimerSession.create(100, 0).id).toBe('11111111-1111-4111-8111-111111111111');
	});
	...
	it('falls back to a generated UUID when randomUUID is unavailable', () => {
		...
		try {
			expect(TimerSession.create(100, 0).id).toMatch(
